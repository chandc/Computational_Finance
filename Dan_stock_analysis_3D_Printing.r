...
#
#  Dassault Systemes S.A. (ADR)(PINK:DASTY)
#  Stratasys (SSYS) is a leading maker of 3D printers
#  3D Systems (DDD)
#
# get monthly adjusted closing price data from Yahoo
...
#
DASTY.prices = get.hist.quote(instrument="DASTY", start="2012-01-01",
                             end="2012-12-31", quote="AdjClose",
                             provider="yahoo", origin="1970-01-01",
                             compression="m", retclass="zoo")
# change class of time index to yearmon which is appropriate for monthly data
# index() and as.yearmon() are functions in the zoo package 
#                             
index(DASTY.prices) = as.yearmon(index(DASTY.prices))
                             
class(DASTY.prices)
colnames(DASTY.prices)
start(DASTY.prices)
end(DASTY.prices)

SSYS.prices = get.hist.quote(instrument="SSYS", start="2012-01-01",
                             end="2012-12-31", quote="AdjClose",
                             provider="yahoo", origin="1970-01-01",
                             compression="m", retclass="zoo")
index(SSYS.prices) = as.yearmon(index(SSYS.prices))

DDD.prices = get.hist.quote(instrument="DDD", start="2012-01-01",
                             end="2012-12-31", quote="AdjClose",
                             provider="yahoo", origin="1970-01-01",
                             compression="m", retclass="zoo")
index(DDD.prices) = as.yearmon(index(DDD.prices))

# create merged price data
Prices.z = merge(DASTY.prices, SSYS.prices, DDD.prices)
# rename columns
colnames(Prices.z) = c("DASTY", "SSYS", "DDD")
...
c("DASTY,SSYS","DASTY,DDD","SSYS,DDD")
covhat.vals
rhohat.vals
...
#
?t.test
t.test(Returns.z[,"DASTY"])
t.test(Returns.z[,"SSYS"])
t.test(Returns.z[,"DDD"])
...
#cor.test
# 
cor.test(x=Returns.z[,"DASTY"], y=Returns.z[,"SSYS"])
# 
cor.test(x=Returns.z[,"DASTY"], y=Returns.z[,"DDD"])
# 
cor.test(x=Returns.z[,"SSYS"], y=Returns.z[,"DDD"])

#
# 11. 12-month rolling estimates of mu and sd
#

# rolling analysis 
roll.mu.DASTY = rollapply(Returns.z[,"DASTY"],
  			  FUN=mean, width = 10, align="right")

roll.sd.DASTY = rollapply(Returns.z[,"DASTY"],
				  FUN=sd, width = 10, align="right")
								    
plot(merge(roll.mu.DASTY,roll.sd.DASTY,Returns.z[,"DASTY"]), plot.type="single",
     main="12-month rolling means and sds for DASTY", ylab="Percent per month",
     col=c("blue","red","black"), lwd=2)	
abline(h=0)
legend(x="bottomleft", legend=c("Rolling means","Rolling sds", "DASTY returns"), 
       col=c("blue","red","black"), lwd=2)
